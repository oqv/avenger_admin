module MyAdmin
    ...
    def can?(user)
    	...
    	self.models.each do |model|
    		can ||= model.my_admin.can?(:list, user)
    	end
    	can
      #not .find { |model| model.my_admin.can?(:list, user) }.blank?
    end
    ...
    def title
      I18n.t!("mongoid.applications.my_admin.#{self.key}") rescue self.key.titleize
    end
